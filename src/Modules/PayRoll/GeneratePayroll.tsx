...
const GeneratePayroll: React.FC = () => {
  ...
  const handleGeneratePayslip = async (values: {
    ...
    try {
      ...
      // Check if we are allowed to view this payslip
      // By checking if it is closed
      const { payrollPeriods } = await dispatch(fetchPayRollPeriods()).unwrap();
      ...
      const periodToView = payrollPeriods.find((period) => {
        const date = new Date(period.periodStartDate);
        return (
          date.getFullYear() === values.year.year() &&
          date.getMonth() === values.month.month()
        );
      });

      if (!periodToView || !periodToView.closed) {
        return api.warning({
          duration: 3,
          message: "Sorry",
          style: {
          ...
          description:
            "Unfortunately, the period requested does not have a payslip yet",
        });
      }
      ...
      if (response) {
        ...
        api.success({
          message: "Success",
          description:
            "Payslip generated successfully. Please enter your password to view your payslip",
          style: {
            borderColor: "#52c41a",
            color: "#fff",
            fontWeight: "semibold",
    ...
    <>
      ...
      >
        <div style={{ position: "relative" }}>
          ...
          <Form layout="vertical" form={form} onFinish={handleGeneratePayslip}>
            <Row gutter={16}>
