...
import { changePassword, resetPassword, selectAuth, selectForgetPassword } from '../features/auth/authSlice';
...
const ResetPassword: React.FC = () => {
    ...
    const staffNumber = new URLSearchParams(window.location.search).get('staffNo')
    const { error, status, message } = useAppSelector(selectForgetPassword);
    const [api, contextHolder] = notification.useNotification();


    const onFinish = (values: { staffNumber: string | null; otpCode: string; newPassword: string; confirmPassword: string }) => {

        dispatch(resetPassword({ staffNo: staffNumber, randomKey: values.otpCode, newPassword: values.newPassword })).unwrap()
            .then((res) => {
                api.success({
                    message: "Success",
